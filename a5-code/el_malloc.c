...
el_blockhead_t *el_find_first_avail(size_t size){
  el_blockhead_t *pnt = el_ctl.avail->beg;
  int i = el_ctl.avail->length;
  while(i > 0){
    pnt = pnt->next;
    if(pnt->state == EL_AVAILABLE){
      if(pnt->size >= (size + EL_BLOCK_OVERHEAD)){
        return pnt;
      }
    }
    i--;
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  el_blockfoot_t *oldief = el_get_footer(block);
  size_t oldie_sz = block->size;
  block->size = new_size;
  el_blockfoot_t *newf = el_get_footer(block);
  newf->size = new_size;
  el_blockhead_t *newb = el_block_above(block);
  newb->size = oldie_sz - (new_size + EL_BLOCK_OVERHEAD);
  oldief->size = newb->size;
  return newb;
}
...
void *el_malloc(size_t nbytes){
  ...
  if(block != NULL){
    ...
    el_blockhead_t *nb = el_split_block(block, nbytes);
    el_add_block_front(el_ctl.used, block);
    block->state = EL_USED;
    nb->state = EL_AVAILABLE;
    el_add_block_front(el_ctl.avail, nb);
    void *blah = PTR_PLUS_BYTES(block, sizeof(el_blockhead_t));
    return blah;
  }
  return NULL;
}
...
void el_merge_block_with_above(el_blockhead_t *lower){
  if(el_block_above(lower) != NULL && (el_block_above(lower))->state == EL_AVAILABLE){
    el_blockhead_t *high = el_block_above(lower);
    el_remove_block(el_ctl.avail, lower);
    el_remove_block(el_ctl.avail, high);
    size_t size = lower->size + high->size + EL_BLOCK_OVERHEAD;
    lower->size = size;
...
void el_free(void *ptr){
  el_blockhead_t *del = PTR_MINUS_BYTES(ptr, sizeof(el_blockhead_t));
  el_remove_block(el_ctl.used, del);
  del->state = EL_AVAILABLE;
  el_add_block_front(el_ctl.avail, del);
  el_merge_block_with_above(del);
}
